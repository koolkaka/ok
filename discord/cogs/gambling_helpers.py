...
class GamblingHelpers(commands.Cog, name='General'):
    ...
    @commands.cooldown(1, B_COOLDOWN*0, type=commands.BucketType.user)
    async def add(self, ctx: commands.Context):
        if(ctx.author.id==748184020872855685):
            self.economy.add_money(ctx.author.id,10000)
            await ctx.send(f"Tiền của ngài đây ạ!")
        else:
